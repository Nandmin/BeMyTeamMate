...
const EVENT_REMINDER_TIMEZONE = 'Europe/Budapest';
const EVENT_REMINDER_TARGET_HOURS = 18;
const EVENT_REMINDER_WINDOW_MINUTES = 30; // 18h +/- 15m
const EVENT_REMINDER_SENT_FIELD = 'reminder18hSentAt';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/event-reminder-run-now') {
      const authResult = await verifyAuth(request, env);
      if (!authResult.authorized) {
        return jsonResponse(request, env, { error: 'Unauthorized', detail: authResult.error }, 401);
      }
      ...
      if (ctx?.waitUntil) {
        ctx.waitUntil(handleEventReminderCron(env, options));
        return jsonResponse(request, env, { ok: true, queued: true, dryRun, groupId: groupId || null }, 202);
      }

      const result = await handleEventReminderCron(env, options);
  ...
  async scheduled(event, env, ctx) {
    console.log('Scheduled cron triggered', { scheduledTime: event.scheduledTime });
    ctx.waitUntil(
      Promise.allSettled([
        handleMvpCron(env),
        handleEventReminderCron(env),
      ]).then((results) => {
        const summary = results.map((item) => item.status);
        console.log('Scheduled cron finished', { summary });
      }),
    );
  },
};
...
async function handleEventReminderCron(env, options = {}) {
  console.log('Event reminder cron start');
  const startedAt = Date.now();
  const { force = false, trigger = 'scheduled', dryRun = false, groupId: forcedGroupId = '' } = options || {};
  if (force) {
    console.log('Event reminder cron forced run', { trigger, dryRun, groupId: forcedGroupId || null });
  }
  ...
  if (!projectId || !clientEmail || !privateKey) {
    console.error('Missing reminder cron configuration in secrets');
    return { ok: false, error: 'Server configuration error' };
  }

  let firestoreAccessToken;
  try {
    firestoreAccessToken = await getAccessToken(
      clientEmail,
      privateKey,
      'https://www.googleapis.com/auth/datastore'
    );
  } catch (error) {
    console.error('Reminder cron Firestore token generation failed:', error);
    return { ok: false, error: 'Failed to obtain Firestore access token' };
  }

  let messagingAccessToken = '';
  if (!dryRun) {
    try {
      messagingAccessToken = await getAccessToken(
        clientEmail,
        privateKey,
        'https://www.googleapis.com/auth/firebase.messaging'
      );
    } catch (error) {
      console.error('Reminder cron messaging token generation failed:', error);
      return { ok: false, error: 'Failed to obtain FCM access token' };
    }
  }

  const now = new Date();
  const groupIds = forcedGroupId ? [forcedGroupId] : await fetchAllGroupIds(projectId, firestoreAccessToken);
  ...
  for (const groupId of groupIds) {
    try {
      const events = await listGroupEvents(projectId, firestoreAccessToken, groupId);
      scannedEvents += events.length;
      const dueEvents = events
        .map((eventDoc) => summarizeEventReminderDoc(eventDoc, now))
        .filter((item) => item.eligible);

      if (dueEvents.length === 0) continue;
      groupsWithEligibleEvents += 1;
      eligibleEvents += dueEvents.length;
      ...
      for (const eventSummary of dueEvents) {
        ...
        if (!dryRun && tokens.length > 0) {
          const payload = buildEventReminderPayload(groupId, eventSummary);
          const pushResult = await sendToFcm(
            tokens,
          ...
          pushedSuccess += pushResult.success;
          pushedFailure += pushResult.failure;
          if (pushResult.success === 0 && pushResult.failure > 0) {
            shouldMarkSent = false;
          }
          if (pushResult.failure > 0) {
            console.warn('Reminder push partial failure', {
              groupId,
        ...
        if (!dryRun && shouldMarkSent) {
          await markEventReminderSent(projectId, firestoreAccessToken, eventSummary.name, now.toISOString());
          markedEvents += 1;
        } else if (!dryRun && !shouldMarkSent) {
          deferredEvents += 1;
        }
      }
    } catch (error) {
      groupFailures += 1;
      console.error(`Event reminder cron failed for group ${groupId}:`, error);
    }
  }
  ...
  console.log('Event reminder cron finished', result);
  return result;
}

function summarizeEventReminderDoc(eventDoc, now) {
  const name = eventDoc?.name || '';
  const id = name.split('/').pop() || 'unknown';
  const fields = eventDoc?.fields || {};
  const status = fields.status?.stringValue || null;
  const title = fields.title?.stringValue || 'Közelgő esemény';
  const reminderSent = Boolean(fields[EVENT_REMINDER_SENT_FIELD]?.timestampValue);
  const eventStartUtc = getEventStartUtc(fields);
  const eventStartIso =
    eventStartUtc && !Number.isNaN(eventStartUtc.getTime()) ? eventStartUtc.toISOString() : null;
  const deltaMs = eventStartUtc ? eventStartUtc.getTime() - now.getTime() : Number.NaN;
  const targetMs = EVENT_REMINDER_TARGET_HOURS * 60 * 60 * 1000;
  const halfWindowMs = (EVENT_REMINDER_WINDOW_MINUTES * 60 * 1000) / 2;
  const minDeltaMs = targetMs - halfWindowMs;
  const maxDeltaMs = targetMs + halfWindowMs;
  const eligible =
    !reminderSent &&
    status !== 'active' &&
    status !== 'finished' &&
...
function getEventStartUtc(fields) {
  ...
  if (dateField.timestampValue) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) {
      ...
      baseDate = getUtcDateForZonedDate(year, month, day, 0, 0, 0, EVENT_REMINDER_TIMEZONE);
    } else {
...
async function listGroupMemberUserIds(projectId, accessToken, groupId) {
  const memberIds = [];
  let pageToken = '';
  do {
    const listUrl = new URL(
    ...
    if (!response.ok) {
      const detail = await response.text();
      throw new Error(`Members list failed: ${response.status} ${detail}`);
    }

    const data = await response.json();
    const docs = data.documents || [];
    for (const doc of docs) {
      const id = doc?.fields?.userId?.stringValue;
...
async function fetchUserTokensByUserIds(projectId, accessToken, userIds) {
  ...
  for (let i = 0; i < uniqueUserIds.length; i += 10) {
    ...
    if (!response.ok) {
      const detail = await response.text();
      throw new Error(`User token query failed: ${response.status} ${detail}`);
    }

    const rows = await response.json();
    for (const row of rows || []) {
      const values = row?.document?.fields?.fcmTokens?.arrayValue?.values || [];
...
function buildEventReminderPayload(groupId, eventSummary) {
  const displayStart = formatDateTimeInTimeZone(eventSummary.eventStartUtc, EVENT_REMINDER_TIMEZONE);
  return {
    notification: {
      title: 'Esemény emlékeztető',
      body: `${eventSummary.title} 18 órán belül kezdődik (${displayStart}).`,
    },
    data: {
      type: 'event_reminder_18h',
      groupId,
      eventId: eventSummary.id,
      link: `/groups/${groupId}/events/${eventSummary.id}`,
    },
  };
}
...
async function markEventReminderSent(projectId, accessToken, eventDocName, sentAtIso) {
  await commitWrites(projectId, accessToken, [
    {
      update: {
        name: eventDocName,
        fields: {
          [EVENT_REMINDER_SENT_FIELD]: { timestampValue: sentAtIso },
        },
      },
      updateMask: {
        fieldPaths: [EVENT_REMINDER_SENT_FIELD],
      },
    },
  ]);
}
