...
export class EventDetailPage {
  ...
  private autoGeneratedTeamsForFingerprint = signal<string>('');
  ...
  constructor() {
    ...
    // In planned phase, ensure teams (and thus balance) are available without extra queries.
    // We only auto-generate when both teams are empty and the attendee set changed.
    effect(() => {
      const event = this.event();
      const attendees = this.attendingMembers();
      if (!event) return;
      if (event.status === 'active' || event.status === 'finished') return;
      if (attendees.length < 2) return;

      const teamsEmpty = this.teamA().length === 0 && this.teamB().length === 0;
      if (!teamsEmpty) return;

      const fingerprint = attendees
        .map((a) => a.userId)
        .sort()
        .join('|');
      if (!fingerprint || fingerprint === this.autoGeneratedTeamsForFingerprint()) return;

      this.generateTeams();
      this.autoGeneratedTeamsForFingerprint.set(fingerprint);
    });
