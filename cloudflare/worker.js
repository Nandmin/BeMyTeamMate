...
const MAX_MATCH_PLAYER_STAT = 30;
const MATCH_DEFAULT_ELO = 1200;
const MATCH_ELO_K_FACTOR = 32;
const MATCH_GOAL_BONUS = 3;
const MATCH_ASSIST_BONUS = 2;
...
function getHttpStatusFromError(error, fallback = 500) {
...
function toBoundedInt(value, min = 0, max = MAX_MATCH_PLAYER_STAT) {
...
function parseFirestoreNumberValue(field, fallback = null) {
...
function parseRatingSnapshotField(field) {
...
async function canFinalizeGroupMatchResult(projectId, accessToken, groupId, userId) {
...
async function fetchUserElos(projectId, accessToken, userIds) {
  const elos = new Map();
...
async function handleFinalizeMatchResults(request, env) {
  const authResult = await verifyAuth(request, env);
  if (!authResult.authorized) {
    return jsonResponse(request, env, { error: 'Unauthorized', detail: authResult.error }, 401);
  }
